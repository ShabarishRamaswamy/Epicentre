...
const deleteWebhooks = (token: string, appName: string) => {
  return new Promise((resolve, reject) => {
    ...
    request.get(options, (error, response, body) => {
      if (error) {
        logger.log('error','Error getting previous Heroku webhooks');
        ...
        if (bodyJSON.length === 0) {
          logger.log('error','There are no previous Heroku webhooks');
          resolve(true);
        }
        /**
        ...
        bodyJSON.forEach((webhook: any) => {
          ...
          if (res.statusCode !== 200) {
            error = true;
            logger.log('error','Error deleting a previous Heroku webhook');
            resolve(false);
          }
        });
        if (!error) {
          logger.log('error','Successfully deleted all previous Heroku webhooks');
          resolve(true);
        }
      }
    });
  ...
  deleteWebhooks(token, appName).then((result) => {
    if (result) {
      ...
      request.post(options, (error, response, body) => {
        if (error) {
          logger.log('error','Error creating Heroku webhook');
          logger.log('error', error);
        } else {
          logger.log('info','Successfully created Heroku webhook');
        }
      });
    } else {
      logger.log('error','Cannot create Heroku webhook due to previous deletion error');
    }
  });
};
